...
`claude-plan.yml` is a thin container shim: it pulls the shared AI-Implement runner image and executes `run-planning.js` inside it, so planning runs identically on GHA, Fly, and local Docker. Note that Bedrock planning (`provider=bedrock`) is GHA-only — Fly and local Docker execution paths do not carry AWS OIDC credentials and will reject Bedrock requests.
