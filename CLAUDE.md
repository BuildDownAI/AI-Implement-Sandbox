...
`claude-implement.yml` accepts a `runner_image` `workflow_dispatch` input. The orchestrator resolves the image via `resolveDispatchRunnerImage` (checking `.ai-implement/image.yml` in the target repo, then `SESSION_IMAGE` on the orchestrator) and forwards it only when explicit — repos without a re-synced workflow are protected from 422 "unexpected inputs" errors by the "only forward when explicit" guard. The `validate-runner-image` job in `claude-implement.yml` applies an allowlist before running the implementation job inside the specified container.

`claude-plan.yml` **also** runs the runner image as a container (post-AII-125 re-sync) and honors the same per-repo override and orchestrator channel as implementation. The `dispatchPlanning` GHA path calls `resolveDispatchRunnerImage` and forwards `runner_image` to `claude-plan.yml` with the same "only when explicit" semantics, closing the channel-parity gap between planning and implementation. A target repo must have re-synced `claude-plan.yml` from the post-AII-125 template before the forwarded `runner_image` will be accepted — re-syncing is required for end-to-end functionality.
